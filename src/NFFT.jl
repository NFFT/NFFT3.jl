...
mutable struct NFFT{D}
	...
	function NFFT{D}(N::NTuple{D,Int32},M::Int32,n::NTuple{D,Int32},m::Int32,f1::UInt32,f2::UInt32) where D
	# create plan object
...
function NFFT(N::NTuple{D,Integer},M::Integer,n::NTuple{D,Integer},m::Integer=Int32(default_window_cut_off),f1::UInt32=(D > 1 ? f1_default : f1_default_1d),f2::UInt32=f2_default) where {D}
    # safety checks
	if any(x->x<=0,N)
		error("Every entry of N has to be an even, positive integer." )
	end
	...
	NFFT{D}(NTuple{D,Int32}(N),Int32(M),NTuple{D,Int32}(n),Int32(m),(f1 | MALLOC_X | MALLOC_F_HAT | MALLOC_F | FFTW_INIT),f2)
end
...
function nfft_init(p::NFFT{D}) where {D}
	...
	# initialize values
	ccall(("jnfft_init", lib_path_nfft),Nothing,(Ref{nfft_plan},Int32,Ref{Int32},Int32,Ref{Int32},Int32,UInt32,UInt32),ptr,D,Nv,p.M,n,p.m,p.f1,p.f2)
...
function Base.setproperty!(p::NFFT{D},v::Symbol,val) where {D}
	...
	if v == :x
		...
		ptr = ccall(("jnfft_set_x",lib_path_nfft),Ptr{Float64},(Ref{nfft_plan},Ref{Cdouble}),p.plan,val)
    ...
    elseif v == :f
		...
		ptr = ccall(("jnfft_set_f",lib_path_nfft),Ptr{ComplexF64},(Ref{nfft_plan},Ref{ComplexF64}),p.plan,val)
		Core.setfield!(p,v,ptr)
	...
	elseif v == :fhat
		...
		ptr = ccall(("jnfft_set_fhat",lib_path_nfft),Ptr{ComplexF64},(Ref{nfft_plan},Ref{ComplexF64}),p.plan,val)
		Core.setfield!(p,v,ptr)
...
function trafo_direct(P::NFFT{D}) where {D}
	...
	ptr = ccall(("jnfft_trafo_direct",lib_path_nfft),Ptr{ComplexF64},(Ref{nfft_plan},),P.plan)
...
function adjoint_direct(P::NFFT{D}) where {D}
	...
	ptr = ccall(("jnfft_adjoint_direct",lib_path_nfft),Ptr{ComplexF64},(Ref{nfft_plan},),P.plan)
...
function adjoint(P::NFFT{D}) where {D}
	...
	ptr = ccall(("jnfft_adjoint",lib_path_nfft),Ptr{ComplexF64},(Ref{nfft_plan},),P.plan)
	Core.setfield!(P,:fhat,ptr)
end
