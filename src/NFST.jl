...
mutable struct NFST{D}
	...
	function NFST{D}(N::NTuple{D,Int32},M::Int32,n::NTuple{D,Int32},m::Int32,f1::UInt32,f2::UInt32) where D
	# create plan object
...
function NFST(N::NTuple{D,Integer},M::Integer,n::NTuple{D,Integer},m::Integer=Int32(8),f1::UInt32=(D > 1 ? f1_default : f1_default_1d),f2::UInt32=f2_default) where {D}
	...
	NFST{D}(NTuple{D,Int32}(N),Int32(M),NTuple{D,Int32}(n),Int32(m),(f1 | MALLOC_X | MALLOC_F_HAT | MALLOC_F | FFTW_INIT),f2)
end
...
function nfst_init(p::NFST{D}) where {D}
	...
	# initialize values
	ccall(("jnfst_init", lib_path_nfst),Nothing,(Ref{nfst_plan},Int32,Ref{Int32},Int32,Ref{Int32},Int32,UInt32,UInt32),ptr,D,Nv,p.M,n,p.m,p.f1,p.f2)
...
function Base.setproperty!(p::NFST{D},v::Symbol,val) where {D}
	...
	if v == :x
		...
		ptr = ccall(("jnfst_set_x",lib_path_nfst),Ptr{Float64},(Ref{nfst_plan},Ref{Cdouble}),p.plan,val)
    ...
    elseif v == :f
		...
		ptr = ccall(("jnfst_set_f",lib_path_nfst),Ptr{Float64},(Ref{nfst_plan},Ref{Float64}),p.plan,val)
		Core.setfield!(p,v,ptr)
	...
	elseif v == :fhat
		...
		ptr = ccall(("jnfst_set_fhat",lib_path_nfst),Ptr{Float64},(Ref{nfst_plan},Ref{Float64}),p.plan,val)
		Core.setfield!(p,v,ptr)
...
function trafo_direct(P::NFST{D}) where {D}
	...
	ptr = ccall(("jnfst_trafo_direct",lib_path_nfst),Ptr{Float64},(Ref{nfst_plan},),P.plan)
...
function adjoint_direct(P::NFST{D}) where {D}
	...
	ptr = ccall(("jnfst_adjoint_direct",lib_path_nfst),Ptr{Float64},(Ref{nfst_plan},),P.plan)
