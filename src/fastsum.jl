...
mutable struct FASTSUM
	...
	function FASTSUM( d::Integer, N::Integer, M::Integer, n::Integer, p::Integer, kernel::String, c::Vector{<:Real}, eps_I::Real, eps_B::Real, nn_x::Integer, nn_y::Integer, m_x::Integer, m_y::Integer, flags::UInt32 )
		new( d, N, M, n, p, kernel, Vector{Float64}(c), eps_I, eps_B, nn_x, nn_y, m_x, m_y, false, false, flags )
	end
end #struct fastsumplan

function FASTSUM( d::Integer, N::Integer, M::Integer, n::Integer, p::Integer, kernel::String, c::Real, eps_I::Real, eps_B::Real, nn::Integer, m::Integer )
...
function finalize_plan( p::FASTSUM )
	...
	if !p.finalized
		ccall( ("jfastsum_finalize",lib_path_fastsum), Nothing, (Ref{fastsum_plan},), p.plan )
...
function Base.setproperty!( p::FASTSUM, v::Symbol, val )
	...
	if v == :x
		...
		ptr = ccall( ("jfastsum_set_x", lib_path_fastsum), Ptr{Float64}, (Ref{fastsum_plan},Ref{Cdouble}), p.plan, val )
	 ...
	 elseif v == :y
		...
		ptr = ccall( ("jfastsum_set_y", lib_path_fastsum), Ptr{Float64}, (Ref{fastsum_plan},Ref{Cdouble}), p.plan, val )
		Core.setfield!( p, v, ptr )
   ...
   # edit source coefficients
	elseif v == :alpha
		...
		ptr = ccall( ("jfastsum_set_alpha", lib_path_fastsum), Ptr{ComplexF64}, (Ref{fastsum_plan},Ref{ComplexF64}), p.plan, val )
		Core.setfield!( p, v, ptr )
...
function trafo( p::FASTSUM )
	...
	ptr = ccall( ( "jfastsum_trafo", lib_path_fastsum), Ptr{ComplexF64}, (Ref{fastsum_plan},), p.plan )
...
function trafo_exact( p::FASTSUM )
	...
	ptr = ccall( ("jfastsum_exact", lib_path_fastsum), Ptr{ComplexF64}, (Ref{fastsum_plan},), p.plan )
	Core.setfield!( p, :f, ptr )
