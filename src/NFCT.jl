...
mutable struct NFCT{D}
	...
	function NFCT{D}(N::NTuple{D,Int32},M::Int32,n::NTuple{D,Int32},m::Int32,f1::UInt32,f2::UInt32) where D
	# create plan object
...
function NFCT(N::NTuple{D,Integer},M::Integer,n::NTuple{D,Integer},m::Integer=Int32(8),f1::UInt32=(D > 1 ? f1_default : f1_default_1d),f2::UInt32=f2_default) where {D}
	# safety checks
	if any(x->x<=0,N)
		error("Every entry of N has to be an even, positive integer." )
	end
	...
	NFCT{D}(NTuple{D,Int32}(N),Int32(M),NTuple{D,Int32}(n),Int32(m),(f1 | MALLOC_X | MALLOC_F_HAT | MALLOC_F | FFTW_INIT),f2)
end
...
function nfct_init(p::NFCT{D}) where {D}
	...
	# initialize values
	ccall(("jnfct_init", lib_path_nfct),Nothing,(Ref{nfct_plan},Int32,Ref{Int32},Int32,Ref{Int32},Int32,UInt32,UInt32),ptr,D,Nv,p.M,n,p.m,p.f1,p.f2)
...
function Base.setproperty!(p::NFCT{D},v::Symbol,val) where {D}
	...
	if v == :x
		...
		ptr = ccall(("jnfct_set_x",lib_path_nfct),Ptr{Float64},(Ref{nfct_plan},Ref{Cdouble}),p.plan,val)
    ...
    elseif v == :f
		...
		ptr = ccall(("jnfct_set_f",lib_path_nfct),Ptr{Float64},(Ref{nfct_plan},Ref{Float64}),p.plan,val)
		Core.setfield!(p,v,ptr)
	...
	elseif v == :fhat
		...
		ptr = ccall(("jnfct_set_fhat",lib_path_nfct),Ptr{Float64},(Ref{nfct_plan},Ref{Float64}),p.plan,val)
		Core.setfield!(p,v,ptr)
...
function trafo_direct(P::NFCT{D}) where {D}
	...
	ptr = ccall(("jnfct_trafo_direct",lib_path_nfct),Ptr{Float64},(Ref{nfct_plan},),P.plan)
...
function adjoint_direct(P::NFCT{D}) where {D}
	...
	ptr = ccall(("jnfct_adjoint_direct",lib_path_nfct),Ptr{Float64},(Ref{nfct_plan},),P.plan)
