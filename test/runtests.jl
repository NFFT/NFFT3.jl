...
F = [ exp(-2*pi*im*sum(X[:,j]'*I[l])) for j in 1:M, l in 1:prod(N) ]
...
#generate Fourier matrix
F = [ cos(2*pi*X[:,j][1]*I[l][1])*cos(2*pi*X[:,j][2]*I[l][2])*cos(2*pi*X[:,j][3]*I[l][3]) for j in 1:M, l in 1:prod(N) ]
...
#generate Fourier matrix
F = [ sin(2*pi*X[:,j][1]*I[l][1])*sin(2*pi*X[:,j][2]*I[l][2])*sin(2*pi*X[:,j][3]*I[l][3]) for j in 1:M, l in 1:prod(collect(N).-1) ]
